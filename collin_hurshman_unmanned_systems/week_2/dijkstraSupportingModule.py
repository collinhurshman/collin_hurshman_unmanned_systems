...
class DijkstraPath():
    ...
    def __checkValidity(self, nodeObj): #a private class method for checking validity of nodes
        ...
        for obs in self.obsList:
            if self.distance(obs,(nodeObj.X,nodeObj.Y)) <= self.radius:
                #notice the <= precludes the robot from rubbing directly against an object
                flag = False
        if nodeObj.X < 0 or nodeObj.X > self.grid.Xmax:#check against size of grid
            flag = False
        if nodeObj.Y < 0 or nodeObj.Y > self.grid.Ymax:
            flag = False
        return flag
    ...
    def showBaseGrid(self):
        ...
        plt.title('Configuration Space with Node Indices')
        plt.show()
    ...
    def findPath(self, plotDesired:bool = 1):
        ...
        if plotDesired: #only set up a figure if the user requests it
            plt.figure()
            plt.title("Node Costs with Minimum Path Highlighted")
...
startCoords = (0,0)
endCoords = (10,10)
obsCoords = [(1,1),(4,4),(3,4),(5,0),(5,1),(1,7),(2,7),(3,7)]
dijkstra = DijkstraPath(myGrid,obsCoords,startCoords,endCoords,0.4)
